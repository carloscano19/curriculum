        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          >
            <div className="flex items-center gap-3 mb-10">
              ...
              <h2 className="text-3xl font-bold text-slate-900">{t(cvData.common.sections.educationTitle)}</h2>
            </div>
            ...
            <div className="space-y-8 pl-4 border-l-2 border-slate-100">
              {data?.map((item: any, index: number) => (
                <div key={index} className="relative pl-8 group">
                  ...
                  <h3 className="text-xl font-bold text-slate-900">{t(item.degree)}</h3>
          ...
          >
            <div className="bg-slate-50 rounded-3xl p-8 border border-slate-100 h-full">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="text-2xl font-bold text-slate-900">{t((cvData.common.sections as any).languagesTitle)}</h2>
              </div>
                ...
                {cvData.languages.map((lang, index) => (
                  <div key={index}>
                    <div className="flex justify-between items-end mb-2">
                      <span className="font-bold text-slate-900 text-lg">{t(lang.name)}</span>
                      <span className="text-slate-500 text-sm font-medium">{t(lang.level)}</span>
                    </div>
              ...
              <div className="mt-12 p-6 bg-white rounded-2xl border border-slate-100 shadow-sm">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                  {t({
                    en: "Committed to continuous learning and staying ahead of the curve in digital trends, from Web3 protocols to the latest in Generative AI.",
                    es: "Comprometido con el aprendizaje continuo y manteniéndome a la vanguardia de las tendencias digitales, desde protocolos Web3 hasta lo último en IA generativa."
                  })}
                </p>
              </div>
            </div>
